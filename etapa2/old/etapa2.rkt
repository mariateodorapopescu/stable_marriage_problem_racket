...
(define (preferablel pref-list x y)
  (ormap (lambda (L)
           (if (null? L)
               #f
               (or (equal? (list (car L)) (list x))
                   (if (member y (cdr L)) #t #f)
                   )
               ))
         pref-list))
...
(define (numberb mpref wpref engagements)
  (if(null? engagements) 0
        (if(not(better-match-exists? (cdr (car engagements)) (car (car engagements))
           ...
           (add1 (numberb mpref wpref (cdr engagements)))
         (numberb mpref wpref (cdr engagements)))
         ))
(define (numberf mpref wpref engagements)
  (if(null? engagements) (numberb wpref mpref (deep-rev engagements))
        0
         ))

(define (stable-match? engagements mpref wpref)
  (if (= (+ (numberb mpref wpref engagements) (numberf mpref wpref engagements)) (length engagements))
      #t
      #f))
