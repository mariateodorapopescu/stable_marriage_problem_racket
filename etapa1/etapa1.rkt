...
(define (get-women wpref)
  (reverse (ceva wpref '())))
(define (ceva wpref result1)
  (if (null? wpref)
      result1
      (ceva (cdr wpref) (append (list (car (car wpref))) result1) )))
...
(define (get-pref-list pref person)
  (if (null? pref)
      '()
      (if (equal? (list (car (car pref))) (list person))
          (cdr (car pref))
...
(define (preferable? pref-list x y)
  (or (null? pref-list)
       ;; lista vida
      (or (equal? (list (car pref-list)) (list x)) ;; daca x e la inceput
          (and (member y (cdr pref-list)) ;; daca y e dupa x
...
(define (get-partner engagements person)
  ;(if (null? engagements)
  ;    #f
   ;   (if (equal? (list (car (car engagements))) (list person))
   ;       (cdr (car engagements))
    ;      (get-partner (cdr engagements) person)
    ;      ))

  ;; acum fac cu cond pentru ca da
  ;; (cond (cond1 tru1) (cond2 tru2) (else elseval))
  
  (cond ((not(null? engagements))
         (cond ((equal? (list (car (car engagements))) (list person)) (cdr (car engagements)))
               (else (get-partner (cdr engagements) person)))) (else #f))
)
